...
export function classifyEcosystem(files, config) {
  ...
  if (names.length > 0 && names.every((name) => name.startsWith(workflowPrefix) && extensions.some((ext) => name.endsWith(ext)))) {
...
export function validateNpmSemanticChange(basePackage, headPackage, baseLock, headLock, metadata) {
  ...
  if (baseLock?.lockfileVersion !== headLock?.lockfileVersion || baseLock?.name !== headLock?.name || baseLock?.version !== headLock?.version) {
  ...
  if (baseRoot && headRoot && !jsonEqual(stripDependencySections(baseRoot), stripDependencySections(headRoot))) {
  ...
  for (const key of keys) {
    ...
    changes.push({ ecosystem: 'npm', name: oldEntry.name, from: oldEntry.spec, to: newEntry.spec, risk: comparison.risk });
  ...
  for (const change of changes) {
    if (!metadataNames.has(change.name)) reasons.push(`${change.name} changed but is absent from signed Dependabot metadata`);
  }
  ...
  for (const [packagePath, entry] of Object.entries(headPackages)) {
    ...
    if (entry?.hasInstallScript === true && previous?.hasInstallScript !== true && !jsonEqual(previous, entry)) {
...
function parseDockerImageReference(line) {
  ...
  return {
    ...
    tag,
    digest,
    version: { major: Number(tagMatch[1]), minor: Number(tagMatch[2]), patch: Number(tagMatch[3]), raw: `${tagMatch[1]}.${tagMatch[2]}.${tagMatch[3]}` },
    suffix: tagMatch[4] || '',
  };
}
...
export function validateDockerSemanticChange(baseText, headText, metadata, allowedImages = []) {
  ...
  for (const index of differing) {
    if (!/^\s*FROM\s+/i.test(baseLines[index] || '') || !/^\s*FROM\s+/i.test(headLines[index] || '')) {
  ...
  for (const index of differing) {
    ...
    if (allowedImages.length > 0 && !allowedImages.includes(shortName) && !allowedImages.includes(after.image)) {
      reasons.push(`container image ${after.image} is not allowlisted for autonomous updates`);
    }
    if (!before.digest || !after.digest || !/^[a-f0-9]{64}$/i.test(before.digest) || !/^[a-f0-9]{64}$/i.test(after.digest)) {
    ...
    if (before.tag === after.tag && before.digest !== after.digest) {
      changes.push({ ecosystem: 'docker', name: after.image, from: before.tag, to: after.tag, risk: 'digest' });
      continue;
    }
    ...
    changes.push({ ecosystem: 'docker', name: after.image, from: before.tag, to: after.tag, risk: comparison.risk });
    if (!['patch', 'minor'].includes(comparison.risk)) reasons.push(`${after.image}: ${comparison.reason}`);
  }
  const metadataNames = new Set(metadata.map((item) => item.name));
  for (const change of changes) {
    const shortName = change.name.split('/').at(-1);
    if (!metadataNames.has(change.name) && !metadataNames.has(shortName)) reasons.push(`${change.name} changed but is absent from signed Dependabot metadata`);
  }
  return { eligible: reasons.length === 0, reasons: unique(reasons), changes };
}

function parseActionUseLine(line) {
  const match = line.match(/^(\s*-?\s*uses:\s*)([^\s@]+)@([a-f0-9]{40}|[a-f0-9]{64})(\s+#\s*v?(\d+)(?:\.(\d+))?(?:\.(\d+))?\s*)$/i);
  if (!match) return null;
  return {
    action: match[2],
    sha: match[3],
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < baseLines.length; i += 1) {
      ...
      if (before.action !== after.action) {
        reasons.push(`${filename}:${i + 1} changed action identity from ${before.action} to ${after.action}`);
        continue;
      }
      ...
      if (before.version.major !== after.version.major) {
        changes.push({ ecosystem: 'github-actions', name: after.action, from: before.version.raw, to: after.version.raw, risk: 'major' });
      ...
      if (comparison.risk === 'same') {
        if (/semver-patch$/.test(signed.updateType || '')) comparison = { risk: 'patch', reason: 'signed Dependabot patch update with coarse action annotation' };
        else if (/semver-minor$/.test(signed.updateType || '')) comparison = { risk: 'minor', reason: 'signed Dependabot minor update with coarse action annotation' };
      }
      ...
      changes.push({ ecosystem: 'github-actions', name: after.action, from: before.version.raw, to: after.version.raw, risk: comparison.risk });
      if (!['patch', 'minor'].includes(comparison.risk)) reasons.push(`${after.action}: ${comparison.reason}`);
    }
...
export function validateConfig(config) {
  ...
  if (!Number.isInteger(config?.botUserId) || config.botUserId <= 0) errors.push('botUserId must be a positive integer');
  ...
  if (!nonEmpty(config?.gitCommitterName) || !nonEmpty(config?.gitCommitterEmail)) errors.push('git committer identity must be configured');
  ...
  if (!Array.isArray(config?.manualReviewLabels) || config.manualReviewLabels.length === 0) errors.push('manualReviewLabels must be non-empty');
  if (!Array.isArray(config?.requiredWorkflows) || config.requiredWorkflows.length === 0) errors.push('requiredWorkflows must be non-empty');
  const workflowNames = new Set();
  const gateNames = new Set();
  const workflowFiles = new Set();
  for (const item of config?.requiredWorkflows || []) {
    if (!item.workflow || !item.gate || !item.file) errors.push('each required workflow needs workflow, gate, and dispatch file');
    if (item.file && (item.file.includes('/') || !/^[A-Za-z0-9._-]+\.ya?ml$/.test(item.file))) errors.push(`workflow file ${item.file} must be a basename ending in .yml or .yaml`);
  ...
  if (!nonEmpty(config?.ecosystems?.['github-actions']?.workflowPrefix) || !Array.isArray(config?.ecosystems?.['github-actions']?.extensions)) errors.push('github-actions ecosystem policy is incomplete');
...
class GitHubApi {
  ...
  async request(method, url, body) {
    ...
    if (!response.ok) {
      ...
      throw new Error(`GitHub API ${method} ${url} failed (${response.status}): ${message || 'unknown error'}`);
    }
    return payload;
  }

  get(pathname) { return this.request('GET', pathname.startsWith('http') ? pathname : `${this.root}${pathname}`); }
  post(pathname, body) { return this.request('POST', pathname.startsWith('http') ? pathname : `${this.root}${pathname}`, body); }
  patch(pathname, body) { return this.request('PATCH', pathname.startsWith('http') ? pathname : `${this.root}${pathname}`, body); }
  put(pathname, body) { return this.request('PUT', pathname.startsWith('http') ? pathname : `${this.root}${pathname}`, body); }
  ...
  async paginate(pathname, selector = null) {
    ...
    for (let page = 1; page <= this.maxPaginationPages; page += 1) {
      ...
      if (!Array.isArray(pageValues)) throw new Error(`pagination endpoint ${pathname} did not return ${selector || 'an array'}`);
      values.push(...pageValues);
      if (pageValues.length < PAGE_SIZE) return values;
    }
    throw new Error(`pagination safety limit reached for ${pathname} after ${this.maxPaginationPages} page(s)`);
  }

  async fileAt(filename, ref) {
    const encodedPath = filename.split('/').map(encodeURIComponent).join('/');
    const payload = await this.get(`/contents/${encodedPath}?ref=${encodeURIComponent(ref)}`);
    if (payload?.encoding !== 'base64' || typeof payload?.content !== 'string') throw new Error(`Unable to decode ${filename}@${ref}`);
...
async function getPullFiles(api, pull) {
  if (pull.changed_files > 100) throw new Error(`PR changes ${pull.changed_files} files; refusing to paginate an oversized autonomous change`);
  return api.get(`/pulls/${pull.number}/files?per_page=100`);
}

async function getPullCommits(api, pull) {
  if (pull.commits > 100) throw new Error(`PR contains ${pull.commits} commits; refusing oversized autonomous history`);
...
export function validateProvenance({ pull, commits, baseSha, config, now = new Date() }) {
  const reasons = [];
  if (pull.user?.login !== config.botLogin) reasons.push(`PR author is ${pull.user?.login || 'unknown'}, not ${config.botLogin}`);
  if (pull.user?.id !== config.botUserId) reasons.push(`PR author numeric identity is ${pull.user?.id ?? 'unknown'}, expected ${config.botUserId}`);
  if (pull.base?.ref !== config.baseBranch) reasons.push(`base branch is ${pull.base?.ref}, expected ${config.baseBranch}`);
  if (pull.head?.repo?.full_name !== pull.base?.repo?.full_name) reasons.push('Dependabot PR head must be in the same repository');
  if (!String(pull.head?.ref || '').startsWith('dependabot/')) reasons.push('head branch is not a Dependabot branch');
  if (pull.draft) reasons.push('draft PRs are never autonomously merged');
  if (config.automergeEnabled !== true) reasons.push('repository autonomous merge kill switch is disabled');
  const labels = new Set((pull.labels || []).map((label) => typeof label === 'string' ? label : label.name).filter(Boolean));
  for (const label of config.manualReviewLabels || []) if (labels.has(label)) reasons.push(`PR carries manual-review label ${label}`);
  const createdAt = new Date(pull.created_at);
  if (Number.isNaN(createdAt.getTime())) reasons.push('PR creation timestamp is invalid');
  else if (now.getTime() - createdAt.getTime() > config.maxPullRequestAgeDays * 86_400_000) reasons.push(`PR is older than autonomous limit ${config.maxPullRequestAgeDays} day(s)`);
  if (commits.length !== 1 || pull.commits !== 1) reasons.push('autonomous merge requires exactly one Dependabot commit');
  const commit = commits[0];
  if (commit) {
    if (commit.author?.login !== config.botLogin) reasons.push(`commit author is ${commit.author?.login || 'unknown'}, not ${config.botLogin}`);
    if (commit.author?.id !== config.botUserId) reasons.push(`commit author numeric identity is ${commit.author?.id ?? 'unknown'}, expected ${config.botUserId}`);
    if (commit.committer?.login !== config.trustedCommitterLogin) reasons.push(`commit was materialized by ${commit.committer?.login || 'unknown'}, expected ${config.trustedCommitterLogin}`);
    if (commit.commit?.author?.name !== config.botLogin) reasons.push('Git commit author name does not match Dependabot');
    if (commit.commit?.author?.email !== config.botAuthorEmail) reasons.push('Git commit author email does not match canonical Dependabot identity');
    if (commit.commit?.committer?.name !== config.gitCommitterName || commit.commit?.committer?.email !== config.gitCommitterEmail) reasons.push('Git commit committer identity does not match GitHub signing infrastructure');
    if (commit.commit?.verification?.verified !== true || commit.commit?.verification?.reason !== 'valid') reasons.push('Dependabot commit signature is not GitHub-verified as valid');
    if (typeof commit.commit?.verification?.signature !== 'string' || commit.commit.verification.signature.length === 0) reasons.push('Dependabot commit has no verifiable signature material');
    if (!String(commit.commit?.message || '').includes(config.signedOffBy)) reasons.push('Dependabot commit is missing the canonical Signed-off-by trailer');
    if (commit.parents?.length !== 1) reasons.push('Dependabot commit must not be a merge commit');
    if (commit.parents?.[0]?.sha !== baseSha) reasons.push('PR is not rebased directly on the current base branch head');
    if (commit.sha !== pull.head?.sha) reasons.push('PR head SHA does not equal the verified Dependabot commit SHA');
  }
...
export function validateSignedMetadata(commit, config) {
  ...
  const reasons = [];
  if (metadata.length === 0) reasons.push('verified Dependabot commit contains no updated-dependencies metadata');
  for (const item of metadata) {
    if (!item.name || !item.version || !item.updateType) reasons.push('Dependabot metadata entry is incomplete');
    if (!config.allowedUpdateTypes.includes(item.updateType)) reasons.push(`${item.name || 'dependency'} uses non-autonomous update type ${item.updateType || 'unknown'}`);
  }
...
export function workflowIdentityMatches(run, pull, requirement) {
  ...
  const associationMatches = !Array.isArray(run?.pull_requests)
    || run.pull_requests.length === 0
    || run.pull_requests.some((item) => item.number === pull.number);
  return run?.name === requirement.workflow
...
async function validateChangeSemantics({ api, pull, files, ecosystem, metadata, config }) {
  ...
  if (ecosystem === 'npm') {
    const [basePackageText, headPackageText, baseLockText, headLockText] = await Promise.all([
      api.fileAt('package.json', baseRef), api.fileAt('package.json', headRef), api.fileAt('package-lock.json', baseRef), api.fileAt('package-lock.json', headRef),
    ]);
    return validateNpmSemanticChange(JSON.parse(basePackageText), JSON.parse(headPackageText), JSON.parse(baseLockText), JSON.parse(headLockText), metadata);
  }
  if (ecosystem === 'docker') {
    const filename = files[0].filename;
    const [baseText, headText] = await Promise.all([api.fileAt(filename, baseRef), api.fileAt(filename, headRef)]);
    return validateDockerSemanticChange(baseText, headText, metadata, config.ecosystems.docker.allowedImages || []);
  }
  if (ecosystem === 'github-actions') {
    const baseByPath = {};
    const headByPath = {};
    await Promise.all(files.flatMap((file) => [
      api.fileAt(file.filename, baseRef).then((text) => { baseByPath[file.filename] = text; }),
      api.fileAt(file.filename, headRef).then((text) => { headByPath[file.filename] = text; }),
    ]));
    return validateActionsSemanticChange(files, baseByPath, headByPath, metadata, config.manualReviewPaths || []);
  }
...
async function qualificationForHead(api, pull, config) {
  const runs = await api.paginate(`/actions/runs?head_sha=${encodeURIComponent(pull.head.sha)}&event=pull_request`, 'workflow_runs');
  ...
  for (const requirement of config.requiredWorkflows) {
    ...
    if (run.conclusion !== 'success') {
      qualifications.push({ ...requirement, state: `run-${run.conclusion || 'unknown'}`, runId: run.id });
      continue;
    }
    ...
    else if (gate.status !== 'completed') qualifications.push({ ...requirement, state: 'gate-pending', runId: run.id });
    else if (gate.conclusion !== 'success') qualifications.push({ ...requirement, state: `gate-${gate.conclusion || 'unknown'}`, runId: run.id });
  ...
  const anyFailed = qualifications.some((item) => !['success', 'missing', 'pending', 'gate-pending'].includes(item.state));
  return { allSuccess, anyFailed, qualifications };
}

function renderComment({ assessment, config, merged = false, dispatches = [] }) {
  const { pull, provenance, metadataAssessment, ecosystem, semantic, qualification, baseSha } = assessment;
  const state = merged
  ...
  if (semantic.changes?.length) {
    lines.push('**Proven dependency changes**', '', '| Dependency | From | To | Risk |', '| --- | --- | --- | --- |');
    for (const change of semantic.changes) lines.push(`| \`${change.name}\` | \`${change.from}\` | \`${change.to}\` | \`${change.risk}\` |`);
    lines.push('');
  }
  const blockers = unique([...provenance.reasons, ...metadataAssessment.reasons, ...semantic.reasons]);
  if (blockers.length) {
  ...
  if (qualification) {
    lines.push('**Exact-head qualification**', '', '| Workflow | Stable gate | State |', '| --- | --- | --- |');
    for (const item of qualification.qualifications) lines.push(`| \`${item.workflow}\` | \`${item.gate}\` | \`${item.state}\` |`);
    lines.push('');
  }
  if (merged && dispatches.length) {
    lines.push('**Post-merge main requalification dispatch**', '', '| Workflow | Dispatch |', '| --- | --- |');
    ...
    if (failures.length) {
      lines.push('', '⚠️ Merge completed, but one or more explicit `main` requalification dispatches failed. This is an operational incident and must be investigated; the pre-merge qualification remains recorded above.');
    }
    lines.push('');
  }
  lines.push(
...
async function upsertComment(api, pullNumber, marker, body) {
  ...
  const matches = comments.filter((comment) => comment.user?.login === 'github-actions[bot]' && String(comment.body || '').includes(marker));
  if (matches.length > 1) throw new Error(`found ${matches.length} governance status comments; refusing ambiguous idempotent update`);
...
async function assessPull(api, number, config, { includeQualification = true } = {}) {
  ...
  try {
    ...
    return {
      pull, baseSha, files, ecosystem: 'unknown', provenance: fallback,
      metadataAssessment: { eligible: false, reasons: ['provenance could not be established'], metadata: [] },
      semantic: { eligible: false, reasons: ['change semantics were not evaluated'], changes: [] },
      qualification: includeQualification ? { allSuccess: false, anyFailed: false, qualifications: [] } : null,
    };
  }
  ...
  if (files.length > config.maxChangedFiles) semantic.reasons.push(`PR changes ${files.length} files, exceeding autonomous limit ${config.maxChangedFiles}`);
  if (pull.changed_files !== files.length) semantic.reasons.push(`GitHub reports ${pull.changed_files} changed files but ${files.length} were enumerated`);
  if (provenance.eligible && metadataAssessment.eligible && semantic.reasons.length === 0) {
    const evaluated = await validateChangeSemantics({ api, pull, files, ecosystem, metadata: metadataAssessment.metadata, config });
    semantic = { ...evaluated, reasons: unique([...semantic.reasons, ...evaluated.reasons]) };
  } else if (semantic.reasons.length === 0) {
    semantic.reasons.push('semantic auto-merge evaluation skipped because provenance or signed metadata is not eligible');
  }
  const qualification = includeQualification && provenance.eligible && metadataAssessment.eligible && semantic.eligible
    ...
    : includeQualification ? { allSuccess: false, anyFailed: false, qualifications: config.requiredWorkflows.map((item) => ({ ...item, state: 'not-evaluated', runId: null })) } : null;
...
async function maybeMerge(api, assessment, config, allowMerge) {
  ...
  const result = await api.put(`/pulls/${refreshed.pull.number}/merge`, {
    ...
    commit_message: 'Qualified and merged by dependency governance after provenance, semantic-scope, exact-base, CI, extended, security, and documentation gates.',
  });
  const dispatches = [];
  if (result?.merged === true) {
    for (const requirement of config.requiredWorkflows) {
      try {
        await api.post(`/actions/workflows/${encodeURIComponent(requirement.file)}/dispatches`, { ref: config.baseBranch });
        dispatches.push({ workflow: requirement.workflow, file: requirement.file, state: 'requested' });
      } catch (error) {
        dispatches.push({ workflow: requirement.workflow, file: requirement.file, state: 'failed', error: error.message });
      }
    }
  }
...
export function eventPullNumber(event, eventName) {
  if (eventName === 'pull_request_target' || eventName === 'pull_request') return event.pull_request?.number || null;
  if (eventName === 'workflow_dispatch') {
    const value = event.inputs?.['pr-number'];
    return value == null || value === '' ? null : parsePositiveInteger(value, 'workflow_dispatch pr-number');
  }
...
async function processPull(api, number, config, { allowMerge, includeQualification = true }) {
  ...
  const failedDispatches = (mergeAttempt.dispatches || []).filter((item) => item.state !== 'requested');
  if (mergeAttempt.merged && failedDispatches.length) {
    throw new Error(`Merge succeeded but ${failedDispatches.length} post-merge workflow dispatch(es) failed: ${failedDispatches.map((item) => `${item.workflow}: ${item.error}`).join('; ')}`);
  }
