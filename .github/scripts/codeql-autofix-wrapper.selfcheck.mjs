...
test('caller targets only the requested Cypress alerts with bounded scope', () => {
...
test('autofix control plane is protected from autonomous dependency updates', () => {
  for (const protectedPath of [
    ...
    assert.ok(governance.manualReviewPaths.includes(protectedPath), `${protectedPath} must require manual review`);
  }
});
